...
if __name__ == "__main__":
    ...
    print("parameters original :::", parameters.shape)
    parameters = process_parameters(parameters, args.imitation_learning,
                                    state_dim, args.il_noise, augment=args.data_augment)
    print("parameters :::", parameters.shape)
    
    mppca = MPPCA(n_clusters, config[args.task_name]["latent_dim"], n_init=5)
    ...
    rl_model = RLModel(mppca, 
                       context_dim=config[args.task_name]["state_dim"], kl_bound=kl_bound, 
