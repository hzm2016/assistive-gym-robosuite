...
from envs.robosuite.robosuite.utils.control_utils import *
...
class OperationalSpaceController(Controller):
    ...
    def set_goal(self, action, set_pos=None, set_ori=None):
        ...
        if self.impedance_mode == "variable":
            ...
            self.kp = (self.kp_max - self.kp_min) / 2 * (kp + np.ones_like(kp)) + self.kp_min
            # self.kp = np.clip(kp, self.kp_min, self.kp_max)
            # self.kd = 2 * np.sqrt(self.kp) * np.clip(damping_ratio,
                                                     # self.damping_ratio_min, self.damping_ratio_max)
            self.damping_ratio = (self.damping_ratio_max - self.damping_ratio_min)/2 * (damping_ratio + np.ones_like(6)) + \
                                 self.damping_ratio_min
            # self.kd = 2 * np.sqrt(self.kp) * np.clip(damping_ratio,
            #                                          self.damping_ratio_min, self.damping_ratio_max)
        ...
        self.goal_ori = set_goal_orientation(scaled_delta[3:],
                                             ...
                                             set_ori=set_ori)
        # print("ee_pos :", self.ee_pos, scaled_delta[:3])
    ...
    def control_limits(self):
        ...
        if self.impedance_mode == "variable":
            ...
            low = np.concatenate([-np.ones_like(self.damping_ratio_min), -np.ones_like(self.kp_min), self.input_min])
            high = np.concatenate([np.ones_like(self.damping_ratio_max), np.ones_like(self.kp_max), self.input_max])
        elif self.impedance_mode == "variable_kp":
            low = np.concatenate([-np.ones_like(self.kp_min), self.input_min])
            high = np.concatenate([np.ones_like(self.kp_max), self.input_max])
