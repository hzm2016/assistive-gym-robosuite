...
from envs.mujoco.utils.insertion import hole_insertion_samples_unrestricted
from envs.mujoco.utils.kinematics import forwardKin
from envs.mujoco.utils.quaternion import identity_quat
from envs.mujoco.utils.randomize import sample_pose
...
def gen_random_reachable_holes(sim, hole_center, pos_range=0.8, angle_range=np.pi, num_samples=250):
	...
	while i < num_samples:
		...
		try:
			qpos_sol = hole_insertion_samples_unrestricted(sim, nsamples=5, insertion_range=(0, 0.06),
														   raise_on_fail=True)
		except RuntimeError:
			continue
...
if __name__ == "__main__":
	...
	hole_center = kuka_base_pos + np.array([0., 0., .5])
	reachable_holes = gen_random_reachable_holes(sim, hole_base_pos, pos_range=.1, angle_range=np.pi / 4,
												 num_samples=250)
	print('Found {} reachable_holes'.format(len(reachable_holes)))
