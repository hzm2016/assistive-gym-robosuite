...
class AssistiveEnv(gym.Env):
    def __init__(self, robot=None, human=None, task='', obs_robot_len=0, obs_human_len=0, time_step=0.02, frame_skip=5,
        ...
        self.observation_space = spaces.Box(
            low=np.array([-1000000000.0] * (self.obs_robot_len + self.obs_human_len), dtype=np.float32),
            high=np.array([1000000000.0] * (self.obs_robot_len + self.obs_human_len), dtype=np.float32),
            dtype=np.float32)
        self.observation_space_robot = spaces.Box(low=np.array([-1000000000.0] * self.obs_robot_len, dtype=np.float32),
                                                  high=np.array([1000000000.0] * self.obs_robot_len, dtype=np.float32),
                                                  dtype=np.float32)
        self.observation_space_human = spaces.Box(low=np.array([-1000000000.0] * self.obs_human_len, dtype=np.float32),
                                                  high=np.array([1000000000.0] * self.obs_human_len, dtype=np.float32),
                                                  dtype=np.float32)
        ...
        # print(os.path.join(
        #     os.path.join(os.path.join(os.path.dirname(os.path.dirname(os.path.realpath(__file__))), 'envs_assistive'),
        #                  'config.ini')))
        self.configp.read(
            os.path.join(os.path.join(os.path.dirname(os.path.dirname(os.path.realpath(__file__))), 'envs_assistive'),
                         'config.ini'))
    ...
    def update_action_space(self):
        action_len = np.sum(
            [len(a.controllable_joint_indices) for a in self.agents if not isinstance(a, Human) or a.controllable])
        self.action_space.__init__(low=-np.ones(action_len, dtype=np.float32),
                                   high=np.ones(action_len, dtype=np.float32), dtype=np.float32)
    ...
    def create_human(self, controllable=False, controllable_joint_indices=[], fixed_base=False,
        ...
        self.human.init(self.human_creation, self.human_limits_model, fixed_base, human_impairment, gender, None,
                        self.id, self.np_random, mass=mass, radius_scale=radius_scale, height_scale=height_scale)
    ...
    def take_step(self, actions, gains=None, forces=None, action_multiplier=1.0, step_sim=True):
        if gains is None:
        ...
        for i, agent in enumerate(self.agents):
            ...
            if needs_action:
                ...
                if len(action) != agent_action_len:
                    print('Received agent actions of length %d does not match expected action length of %d' % (
                    len(action), agent_action_len))
                    exit()
            ...
            # print("agent_joints_angles ::::", agent_joint_angles)
            ...
            for _ in range(self.frame_skip):
                ...
                if isinstance(agent, Human) and agent.impairment == 'tremor':
                    ...
                    agent_joint_angles = agent.target_joint_angles + agent.tremors * (
            ...
            if isinstance(agent, Robot) and agent.action_duplication is not None:
                # print("agent_joints_angles ::::", agent_joint_angles, agent.action_duplication)
                agent_joint_angles = np.concatenate(
                    [[a] * d for a, d in zip(agent_joint_angles, self.robot.action_duplication)])
    ...
    def take_step_joint(self, actions, joint, gains=None, forces=None, action_multiplier=1.0, step_sim=True):
        if gains is None:
            gains = [a.motor_gains for a in self.agents]
        elif type(gains) not in (list, tuple):
            gains = [gains] * len(self.agents)
        ...
        for i, agent in enumerate(self.agents):
            ...
            if isinstance(agent, Robot):
                ...
                agent.control(agent.controllable_joint_indices, joint, gains[i], forces[i])
            else:
                # print("agent_joints_angles ::::", agent_joint_angles)
                agent.control(agent.controllable_joint_indices, agent_joint_angles, gains[i], forces[i])
    ...
    def human_preferences(self, end_effector_velocity=0, total_force_on_human=0, tool_force_at_target=0,
        ...
        if self.task == 'arm_manipulation':
            ...
            tool_right_pressure = 0 if tool_right_contact_points <= 0 else (
                        arm_manipulation_tool_forces_on_human[0] / tool_right_contact_points)
            tool_left_pressure = 0 if tool_left_contact_points <= 0 else (
                        arm_manipulation_tool_forces_on_human[1] / tool_left_contact_points)
    ...
    def init_robot_pose(self, target_ee_pos, target_ee_orient, start_pos_orient, target_pos_orients, arm='right',
        ...
        for _ in range(max_iterations):
            if self.robot.mobile:
                ...
                self.robot.ik_random_restarts(right=(arm == 'right'), target_pos=target_ee_pos,
                                              target_orient=target_ee_orient,
                                              max_iterations=1000,
                                              max_ik_random_restarts=1000, success_threshold=0.01,
                                              step_sim=False, check_env_collisions=False,
                                              randomize_limits=True, collision_objects=collision_objects)
            else:
                # Use TOC with JLWKI to find an optimal base position for the robot near the person
                base_position, _, _ = self.robot.position_robot_toc(self.task, arm,
                                                                    start_pos_orient,
                                                                    target_pos_orients,
                                                                    self.human,
                                                                    step_sim=False,
                                                                    check_env_collisions=False,
                                                                    max_ik_iterations=100, max_ik_random_restarts=1,
                                                                    randomize_limits=False, right_side=right_side,
    ...
    def setup_camera_rpy(self, camera_target=[-0.2, 0, 0.75], distance=1.5, rpy=[0, -35, 40], fov=60,
    ...
    def create_sphere(self, radius=0.01, mass=0.0, pos=[0, 0, 0], visual=True, collision=True, rgba=[0, 1, 1, 1],
        ...
        body = p.createMultiBody(baseMass=mass, baseCollisionShapeIndex=sphere_collision,
                                 baseVisualShapeIndex=sphere_visual, basePosition=pos,
                                 useMaximalCoordinates=maximal_coordinates, physicsClientId=self.id)
        sphere = Agent()
        sphere.init(body, self.id, self.np_random, indices=-1)
        return sphere
    
    def create_spheres(self, radius=0.01, mass=0.0, batch_positions=[[0, 0, 0]], visual=True, collision=True,
                       rgba=[0, 1, 1, 1]):
        sphere_collision = p.createCollisionShape(shapeType=p.GEOM_SPHERE, radius=radius,
                                                  physicsClientId=self.id) if collision else -1
        sphere_visual = p.createVisualShape(shapeType=p.GEOM_SPHERE, radius=radius, rgbaColor=rgba,
    ...
    def create_agent_from_obj(self, visual_filename, collision_filename, scale=1.0, mass=1.0, pos=[0, 0, 0],
                              orient=[0, 0, 0, 1], rgba=[1, 1, 1, 1], maximal=False):
        visual_shape = p.createVisualShape(shapeType=p.GEOM_MESH, fileName=visual_filename, meshScale=scale,
                                           rgbaColor=rgba, physicsClientId=self.id)
        collision_shape = p.createCollisionShape(shapeType=p.GEOM_MESH, fileName=collision_filename, meshScale=scale,
                                                 physicsClientId=self.id)
