...
class Robot(Agent):
    def __init__(self, controllable_joints,
                 ...
                 right_gripper_indices, left_gripper_indices,
                 gripper_pos,
                 right_tool_joint, left_tool_joint,
                 tool_pos_offset, tool_orient_offset,
                 right_gripper_collision_indices, left_gripper_collision_indices,
                 toc_base_pos_offset, toc_ee_orient_rpy,
                 wheelchair_mounted, half_range=False,
                 controllable_joint_indices=None, action_duplication=None, action_multiplier=1, flags=None):
    ...
    def position_robot_toc(self, task, arms, start_pos_orient, target_pos_orients, human, base_euler_orient=np.zeros(3), max_ik_iterations=200, max_ik_random_restarts=1, randomize_limits=False, attempts=100, jlwki_restarts=1, step_sim=False, check_env_collisions=False, right_side=True, random_rotation=30, random_position=0.5):
        ...
        while iteration < attempts or best_position is None:
            ...
            # random_orientation = np.zeros(3)
            random_orientation = self.get_quaternion([base_euler_orient[0], base_euler_orient[1], base_euler_orient[2]])
            # self.set_base_pos_orient(np.array([-0.85, -0.4, 0]) + self.toc_base_pos_offset[task] + random_pos,
                                     # random_orientation)
            self.set_base_pos_orient(np.array([-0.85, -0.4, 0]) + self.toc_base_pos_offset[task], random_orientation)
    ...
    def get_ee_pose(self):
        ...
        return self.get_pos_orient(self.right_end_effector, convert_to_realworld=False)
