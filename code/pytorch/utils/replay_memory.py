...
class ReplayMemory:
    ...
    def load_dataset(self, data_path, state_dim, action_dim):
        self.storage = dict()
        self.storage['observation'] = np.zeros((self.capacity, state_dim))
        self.storage['action'] = np.zeros((self.capacity, action_dim))
        self.storage['next_observation'] = np.zeros((self.capacity, state_dim))
        self.storage['reward'] = np.zeros((self.capacity, 1))
        self.storage['not_done'] = np.zeros((self.capacity, 1))
