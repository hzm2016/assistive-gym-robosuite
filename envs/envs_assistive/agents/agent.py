...
class Agent:
    ...
    def control(self, indices, target_angles, gains, forces):
        ...
        p.setJointMotorControlArray(self.body, jointIndices=indices, controlMode=p.POSITION_CONTROL,
                                    targetPositions=target_angles, positionGains=gains,
                                    forces=forces, physicsClientId=self.id)

    def torque_control(self, indices, target_torque=None):
        
        p.setJointMotorControlArray(self.body,
                                    jointIndices=indices,
                                    controlMode=p.TORQUE_CONTROL,
                                    forces=target_torque
                                    )
    ...
    def ik(self, target_joint, target_pos, target_orient, ik_indices, max_iterations=1000, half_range=False, use_current_as_rest=False, randomize_limits=False):
        ...
        if target_orient is not None:
            ik_joint_poses = np.array(p.calculateInverseKinematics(self.body, target_joint,
                                                                   ...
                                                                   jointRanges=ik_joint_ranges.tolist(), restPoses=ik_rest_poses.tolist(), maxNumIterations=max_iterations, physicsClientId=self.id))
        else:
            ik_joint_poses = np.array(p.calculateInverseKinematics(self.body, target_joint,
                                                                   targetPosition=target_pos,
                                                                   lowerLimits=ik_lower_limits.tolist(), upperLimits=ik_upper_limits.tolist(), jointRanges=ik_joint_ranges.tolist(),
                                                                   restPoses=ik_rest_poses.tolist(), maxNumIterations=max_iterations, physicsClientId=self.id))
