...
import envs.mujoco.utils.os_utils
from envs.mujoco.utils import transformations
from envs.mujoco.utils.paths import cache_dir
...
class BaseConfig:
	...
	def __init__(self, N_JOINTS, N_LINKS, ROBOT_NAME="robot", use_cython=True):
		...
		abr_control.arms.joco.utils.os_utils.makedirs(self.config_folder)
	...
	def _generate_and_save_function(self, filename, expression, parameters):
		...
		abr_control.arms.joco.utils.os_utils.makedirs(folder)
	...
	def T(self, name, q):
		...
		if self._T_func.get(name, None) is None:
			...
			if T is None and T_func is None:
				...
				# save to file
				abr_control.arms.joco.utils.os_utils.makedirs(
	...
	def _calc_g(self, lambdify=True):
		...
		if g is None and g_func is None:
			...
			# save to file
			abr_control.arms.joco.utils.os_utils.makedirs("%s/g" % self.config_folder)
	...
	def _calc_M(self, lambdify=True):
		...
		if M is None and M_func is None:
			...
			# save to file
			abr_control.arms.joco.utils.os_utils.makedirs("%s/M" % (self.config_folder))
	...
	def _calc_C(self, lambdify=True):
		...
		if C is None and C_func is None:
			...
			# save to file
			abr_control.arms.joco.utils.os_utils.makedirs("%s/C" % self.config_folder)
