...
class FeedingEnv(AssistiveEnv):
    def __init__(self, robot, human):
        super(FeedingEnv, self).__init__(robot=robot, human=human, task='feeding',
                                         obs_robot_len=(18 + len(robot.controllable_joint_indices) - (len(robot.wheel_joint_indices) if robot.mobile else 0)),
                                         obs_human_len=(19 + len(human.controllable_joint_indices)))
    
    def step(self, action):
        if self.human.controllable:
            action = np.concatenate([action['robot'], action['human']])
        
        self.take_step(action, action_multiplier=1.0)

        # self.take_step_joint(action, joint, action_multiplier=1.0)
        ...
        reward = self.config('distance_weight') * reward_distance_mouth_target + \
        ...
        done = self.iteration >= self.config('max_episode_steps')
    ...
    def get_total_force(self):
        """
            get contact force from robot and spoon
        """
    ...
    def _get_obs(self, agent=None):
        ...
        # state of robot
        # robot_obs = np.concatenate([spoon_pos_real, spoon_orient_real, spoon_pos_real - target_pos_real, robot_joint_angles,
        #                             head_pos_real, head_orient_real, [self.spoon_force_on_human]]).ravel()

        robot_obs = np.concatenate(
            [spoon_pos_real, spoon_orient_real, spoon_pos_real - target_pos_real,
             head_pos_real, head_orient_real, [self.spoon_force_on_human]]).ravel()
    ...
    def reset(self):
        ...
        self.build_assistive_env('wheelchair', gender='male', human_impairment=None)
        ...
        self.init_robot_pose(target_ee_pos, target_ee_orient,
                             [(target_ee_pos, target_ee_orient), (self.target_pos, None)], [(self.target_pos, target_ee_orient)],
                             arm='right', tools=[self.tool], collision_objects=[self.human, self.table, self.furniture])
    ...
    def generate_target(self):
        ...
        # print("target_initial", self.target.pos)
        self.update_targets()

    def update_targets(self):
        """
            update target of mouth
        """
        # head_pos, head_orient = self.human.get_pos_orient(self.human.head)
        # # print("head ori :", head_orient)
        # rotate_ori = p.getQuaternionFromEuler([1.57, 0, 0])
        ...
        # # print("target ori :", target_orient)
        # self.target_pos = np.array(target_pos)
        # self.target_orient = np.array(target_orient)
        # self.target.set_base_pos_orient(self.target_pos, [0, 0, 0, 1])
        # # self.target.set_base_pos_orient(self.target_pos + np.array([0.1, -0.2, 0.1]), self.target_orient)
        ...
        self.target_pos_referece, self.target_orient_reference = \
            p.multiplyTransforms(target_pos, target_orient, np.array([-0.0, -0.2, 0.0]), [0, 0, 0, 1], physicsClientId=self.id)
        ...
        self.ee_point = self.create_sphere(radius=0.002, mass=0.0, pos=target_ee_pos, collision=False, rgba=[1, 0, 0, 0.5])
    ...
    def get_context(self):
        ...
        # self.reset()
        # self.update_targets()
        # return self.target_pos + np.array([0.1, -0.2, 0.1]), self.target_orient
