...
class DrinkingEnv(AssistiveEnv):
    ...
    def _get_obs(self, agent=None):
        ...
        robot_obs = np.concatenate(
            [cup_pos_real, cup_orient_real, cup_pos_real - target_pos_real, head_pos_real,
             head_orient_real, [self.cup_force_on_human]]).ravel()
    ...
    def reset(self):
        ...
        # print("target_ee_orient :", target_ee_orient)
    ...
    def update_targets(self):
        ...
        self.target_pos_referece, self.target_orient_reference = p.multiplyTransforms(target_pos, target_orient,
                                                                                      np.array([0.0, -0.2, 0.0]), [0, 0, 0, 1], physicsClientId=self.id)
        ...
        self.target_pos_tool, self.target_orient_tool = \
            p.multiplyTransforms(target_pos, target_orient, np.array([-0.0, -0.1, 0.0]),
                                 [0, 0, 0, 1], physicsClientId=self.id)

        # create a reference target ::: sphere
        self.target_tool = self.create_sphere(radius=0.01, mass=0.0, pos=self.target_pos_tool,
                                              collision=False, rgba=[1, 0, 0, 0.1])

        target_ee_pos, target_ee_ori = self.robot.get_ee_pose()
        self.ee_point = self.create_sphere(radius=0.002, mass=0.0, pos=target_ee_pos, collision=False,
                                           rgba=[1, 0, 0, 0.5])

    # def get_context(self):
    #     """ get target pose """
    #     self.update_targets()
    #     return np.concatenate((self.target_pos, self.target_orient), axis=0)
