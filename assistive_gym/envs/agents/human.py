...
class Human(Agent):
	...
	def init(self, human_creation, limits_model, static_human_base, impairment, gender, config, id, np_random,
		...
		# Initialize human
		self.body = human_creation.create_human(static=static_human_base, limit_scale=self.limit_scale,
												specular_color=[0.1, 0.1, 0.1], gender=self.gender, config=config,
	...
	def setup_joints(self, joints_positions, use_static_joints=True, reactive_force=None, reactive_gain=0.05):
		...
		for j in self.all_joint_indices:
			if use_static_joints and (j not in self.controllable_joint_indices or (
					self.impairment != 'tremor' and reactive_force is None and not self.controllable)):
		...
		self.set_joint_angles([j for j, _ in joints_positions],
							  [np.deg2rad(j_angle) for _, j_angle in joints_positions])
		
		# By default, all joints have motors enabled by default that prevent free motion. Disable these motors.
		for j in self.all_joint_indices:
			p.setJointMotorControl2(self.body, jointIndex=j, controlMode=p.VELOCITY_CONTROL, force=0,
	...
	def enforce_realistic_joint_limits(self):
		...
		if (self.j_right_shoulder_x not in self.controllable_joint_indices) and (
				self.j_left_shoulder_x not in self.controllable_joint_indices):
			return
		right = self.j_right_shoulder_x in self.controllable_joint_indices
		indices = [self.j_right_shoulder_x, self.j_right_shoulder_y, self.j_right_shoulder_z,
				   self.j_right_elbow] if right else [self.j_left_shoulder_x, self.j_left_shoulder_y,
													  self.j_left_shoulder_z, self.j_left_elbow]
