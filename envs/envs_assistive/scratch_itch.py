...
from envs.envs_assistive.env import AssistiveEnv


class ScratchItchEnv(AssistiveEnv):
    def __init__(self, robot, human):
        super(ScratchItchEnv, self).__init__(robot=robot, human=human, task='scratch_itch',
                                             obs_robot_len=(23 + len(robot.controllable_joint_indices) - (len(robot.wheel_joint_indices) if robot.mobile else 0)), obs_human_len=(24 + len(human.controllable_joint_indices)))
    ...
    def _get_obs(self, agent=None):
        ...
        self.total_force_on_human, self.tool_force, self.tool_force_at_target, self.target_contact_pos \
            = self.get_total_force()
        ...
        robot_obs = np.concatenate(
            [tool_pos_real, tool_orient_real, tool_pos_real - target_pos_real, target_pos_real,
             shoulder_pos_real, elbow_pos_real, wrist_pos_real, [self.tool_force]]).ravel()
        ...
        if self.human.controllable:
            ...
            # Co-optimization with both human and robot controllable
            return {'robot': robot_obs, 'human': human_obs, 'robot_joint_angles': robot_joint_angles}
        return robot_obs
    ...
    def reset(self):
        ...
        if self.robot.wheelchair_mounted:
            ...
            self.robot.set_base_pos_orient(wheelchair_pos + np.array(self.robot.toc_base_pos_offset[self.task]), [0, 0, 0])
    ...
    def generate_target(self):
        ...
        rotate_tool_ori = p.getQuaternionFromEuler([0.0, 0.0, np.pi/2])
        # self.target_pos, self.target_orient = p.multiplyTransforms(arm_pos, arm_orient, self.target_on_arm, [0, 0, 0, 1], physicsClientId=self.id)
        self.target_pos, self.target_orient = p.multiplyTransforms(arm_pos, arm_orient, self.target_on_arm,
                                                                   rotate_tool_ori, physicsClientId=self.id)
    ...
    def update_targets(self):
        ...
        rotate_tool_ori = p.getQuaternionFromEuler([0.0, 0.0, 0.0])
        # target_pos, target_orient = p.multiplyTransforms(arm_pos, arm_orient, self.target_on_arm, [0, 0, 0, 1], physicsClientId=self.id)
        target_pos, target_orient = p.multiplyTransforms(arm_pos, arm_orient,
                                                         self.target_on_arm, rotate_tool_ori,
                                                         physicsClientId=self.id)
        self.target_pos = np.array(target_pos)
        self.target.set_base_pos_orient(self.target_pos, [0, 0, 0, 1])
        
        self.target_pos_referece, self.target_orient_reference = \
            p.multiplyTransforms(target_pos, target_orient, np.array([-0.0, 0.0, 0.15]), [0, 0, 0, 1],
                                 physicsClientId=self.id)
        
        # create a reference target ::: sphere
        self.target_reference = self.create_sphere(radius=0.01, mass=0.0, pos=self.target_pos_referece,
                                                   collision=False, rgba=[0, 1, 0, 1])

        rotate_tool_ori = p.getQuaternionFromEuler([0.0, 0.0, 0])
        # target_ee_pos, target_ee_ori = self.robot.get_ee_pose()
        # self.target_ee_pos, self.target_ee_orient = \
        #     p.multiplyTransforms(target_ee_pos, target_ee_ori, np.array([0.00, 0.0, 0.1]),
        #                          np.array(rotate_tool_ori), physicsClientId=self.id)
        # self.ee_point = self.create_sphere(radius=0.01, mass=0.0, pos=self.target_ee_pos, collision=False,
        #                                    rgba=[1, 0, 0, 0.5])

        # target_pos, target_orient = p.multiplyTransforms(head_pos, head_orient, self.mouth_pos, rotate_ori,
        #                                                  physicsClientId=self.id)  # [0, 0, 0, 1]
        self.target_pos_tool, self.target_orient_tool = \
            p.multiplyTransforms(target_pos, target_orient, np.array([0.02, -0.12, 0.0]),
                                 np.array(rotate_tool_ori), physicsClientId=self.id)

        # self.target_tool = self.create_sphere(radius=0.01, mass=0.0, pos=self.target_pos_tool,
        #                                       collision=False, rgba=[1, 0, 0, 0.5])

        self.tool_pos, tool_orient = self.tool.get_pos_orient(1)
        self.target_tool = self.create_sphere(radius=0.015, mass=0.0, pos=self.tool_pos, visual=True, collision=False,
                                              rgba=[0, 1, 1, 1])
        
    def get_context(self):
        return self.target_pos, self.target_orient, \
               np.array(self.target_pos_referece), np.array(self.target_orient_reference), \
               np.array(self.target_pos_tool), np.array(self.target_orient_tool)

    def get_tool_pose(self):
        """
            get_tool_pose
        """
        spoon_pos, spoon_orient = self.tool.get_pos_orient(1)
        # spoon_euler = transforms3d.euler.quat2euler(start_ori, 'sxyz')
        return spoon_pos, spoon_orient