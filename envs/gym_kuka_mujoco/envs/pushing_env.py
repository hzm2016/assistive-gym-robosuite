...
class PushingEnv(kuka_env.KukaEnv):
    ...
    def _get_state_obs(self):
        ...
        obs = np.concatenate([obs, ee_lin_vel_obs, ee_rot_vel_obs])
    ...
    def _reset_state(self):
        ...
        # qpos[self.block_pos_idx] = self.block_target_position.copy()
    ...
    def _reset_target(self):
        ...
        #     self.block_target_position = np.array([0.7, 0.1, 1.2, 0.92387953251, 0, 0, 0.38268343236])
        # if self.random_target:
        x_y = np.random.rand(2) * np.array([0.8, 0.4])
        # print('x_y', x_y)
        x = np.clip(x_y[0], 0.5, 0.8)
        y = np.clip(x_y[1], 0.1, 0.5)
        self.block_target_position = np.array([x, y, 1.2, 0.92387953251, 0, 0, 0.38268343236])
    ...
    def get_context(self):
        ...
        if self.contextual_policy:
            self._reset_target()
            # print("Random set target !!!!", self.block_target_position)
            return self.block_target_position
        else:
